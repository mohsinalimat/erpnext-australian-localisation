...
frappe.ui.form.on("Item", {
    before_save(frm){
        if(australian_localisation_settings.make_tax_category_mandatory){
            for(i=0; i < frm.doc.taxes.length;i++){
                if (frm.doc.taxes[i].tax_category){
                    continue
                }
                else{
                    frappe.throw("Tax Catgory Mandatory")
                }
            }
        }
    }
...
frappe.ui.form.on("Item Tax", {
    async item_tax_template(frm,cdt,cdn){
        ...
        let tax_category = ""
        const { message } = await frappe.db.get_value("Item Tax Template", row.item_tax_template, ["title"])
